...
*   **Incrementality Tracking (`outcomes.py`):** The product thesis is *incrementality* — driving consumption of content people would NOT have found anyway, not frontrunning virality. (Both pick types are still deliberately featured, DJ-style: pre-viral "bangers" and obscure "deep cuts" for pacing/variety.) Each pick's view count is snapshotted when featured (`views_at_feature`); ~7 days later (config: `OUTCOME_CHECK_DAYS`) the hourly runner re-polls YouTube and stores `views_after_7d`. Growth near 1x + high "Never Seen It" = incremental; growth ≥3x = frontran a blowup. `/admin/outcomes` pairs each pick's frozen score breakdown with its delta and feedback tallies — the dataset for empirically tuning the Diamond Score weights. `/admin/sources` aggregates this per source (never-seen rate, avg growth, blowups) to judge which sources are worth curating.
